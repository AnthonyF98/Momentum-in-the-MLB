---
title: "Momentum-in-the-MLB-2022"
...
```{r message=FALSE}
# Define weekly ranges for 2022 regular season
dates_2022 <- data.frame(
  start = c("2022-04-01", "2022-04-08", "2022-04-15", "2022-04-22", "2022-04-29",
  ...
  end = c("2022-04-07", "2022-04-14", "2022-04-21", "2022-04-28", "2022-05-05",
          "2022-05-12", "2022-05-19", "2022-05-26", "2022-06-02",
          "2022-06-09", "2022-06-16", "2022-06-23", "2022-06-30",
          "2022-07-07", "2022-07-14", "2022-07-21", "2022-07-28", "2022-08-04",
          "2022-08-11", "2022-08-18", "2022-08-25", "2022-09-01",
          "2022-09-08", "2022-09-15", "2022-09-22", "2022-09-29", "2022-10-06")
)
...
#statcast_2022 <- purrr::map2_dfr(dates_2022$start, dates_2022$end, get_statcast_data_weekly)

# Optional: Save result
#saveRDS(statcast_2022, "statcast_2022_full.rds")

statcast_2022 <- readRDS("statcast_2022_full.rds")
...
statcast_2022_clean <- statcast_2022 %>%
  select(all_of(cols_to_keep)) %>%
  filter(!is.na(events))  # Remove non-PA rows (e.g. pickoffs, warmups)

# Check result
glimpse(statcast_2022_clean)

unique(statcast_2022$events)
...
statcast_pa_2022 <- statcast_2022_clean %>%
...
statcast_pa_2022 <- statcast_pa_2022 %>%
...
team_momentum_ba <- statcast_pa_2022 %>%
 filter(events %in% c(
    "single", "double", "triple", "home_run",
    "field_out", "strikeout", "grounded_into_double_play",
    "double_play", "strikeout_double_play",
...
sum(statcast_pa_2022$hit, na.rm = TRUE)
...
ggplot(team_comparison %>% filter(!is.na(batting_team)), 
  ...
  labs(
    ...
    subtitle = "Difference in Batting Average When Previous Batter Got a Hit or Sac Fly (2022)",
    x = "Team",
