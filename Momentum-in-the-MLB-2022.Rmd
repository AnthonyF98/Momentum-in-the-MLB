...
team_comparison_22 <- team_momentum_ba %>%
  pivot_wider(
    names_from = momentum,
...
print(team_comparison_22)

#write.csv(team_comparison_22, "team_comparison_22.csv", row.names = FALSE)
...
sum(team_comparison_22$Hits_momentum_0, na.rm = TRUE) + 
sum(team_comparison_22$Hits_momentum_1, na.rm = TRUE)
...
ggplot(team_comparison_22 %>% filter(!is.na(batting_team)), 
  ...
  geom_text(aes(label = sprintf("%+d pts", round(BA_Difference * 1000))), 
            hjust = ifelse(team_comparison_22$BA_Difference > 0, -0.15, 1.1), 
